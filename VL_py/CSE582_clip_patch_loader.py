...
class VideoQADataset(Dataset):
    def __init__(
        ...
        """
        # self.data = pd.read_json(osp.join(data_dir, f'{split}.jsonl'))

        self.data = pd.read_csv(osp.join(data_dir, f'{split}.csv'))
        self.frame_size = json.load(open(osp.join(data_dir, 'frame_size.json')))
    ...
    def __getitem__(self, index):
        ...
        answer_txts = str(cur_sample["answer"])
        answer_id = self.a2id.get(answer_txts, -1)
        # print(answer_id, answer_txts)
        # input()
